...
const Home = () => {
  ...
  const { data: nextPosts, fetchNextPage, hasNextPage } = useGetPosts()
  /*
  const { data: posts, isPending: isPostLoading, isError: isErrorPosts,
     fetchNextPage, hasNextPage } = useGetPosts();
  const { data: creators, isPending: isUserLoading, isError: isErrorCreators } = useGetUsers(10);
  const { ref, inView } = useInView({ triggerOnce: false }); // Ensure continuous trigger
  */
  /*
  useEffect(() => {
